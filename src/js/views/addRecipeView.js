        ...
        <div class="recipe__form">
            ...
            <form enctype="multipart/form-data">
                <div class="form__title">
                    <h2>${
                      edit ? `Edit Recipe Details` : `Add Recipe Details`
                    }</h2>
                </div>  
                <div class="form-group">
                    <label for="recipe name">Recipe Name</label>
                    <input type="text" class="form-control label" name="label" id="recipe__name" placeholder="recipe name" value="${
                      edit ? recipe.label : ""
                    }" required>
                </div>
                <div class="form-group">
                    <label for="iamge">Image</label>
                    <input type="file" class="form-control image" name="image" id="image__url" accept="image/*" ${
                      !edit ? `required` : ""
                    }>
                </div>
                <div class="form-group">
                    <label for="ingredients">Ingredients</label>
                    <textarea type="textarea" rows="3" class="form-control ingredientLines" name="ingredientLines" id="ingredients" placeholder="ex: 1 tbsp oil, 1/2 tbsp salt" required>${
                      edit ? recipe.ingredientLines.toString() : ""
                    }</textarea>
                </div>
                <div class="form-group">
                    <label for="author">Author</label>
                    <input type="text" class="form-control source" name="source" id="author" placeholder="creator name" name" value="${
                      edit ? recipe.source : ""
                    }" required>
                </div>
                <div class="form-group">
                    <label for="total time">Total Time</label>
                    <input type="number" min="1" class="form-control totalTime" name="totalTime" id="total__time" placeholder="only number" name" value="${
                      edit ? recipe.totalTime : ""
                    }" required>
                </div>
                <div class="form-group">
                    <label for="servings">Servings</label>
                    <input type="number" min="1" class="form-control yield" name="yield" id="servings" placeholder="recipe serve for total number of persons" name" value="${
                      edit ? recipe.yield : ""
                    }" required>
                </div>
                <div class="form__button">
                    <input type="button" value=" ${
                      edit ? "Update recipe" : "Add recipe"
                    }" title="${
    edit ? "Update Recipe" : "Add Recipe"
  }" class="btn ${edit ? "edit__recipe-data" : "add__recipe-data"}">
                    <br><br>
