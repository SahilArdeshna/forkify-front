export default class  Likes {
  ...
  addRecipeIntoDb(recipe) {
    ...
    return $.ajax({
      type: "POST",
      url: 'https://forkify--api.herokuapp.com/addLikedRecipe',
      data: recipeForm,
      processData: false,
      contentType: false,
      headers: {
  ...
  async getLikedRecipesFromDB() {
    return $.ajax({
      type: "GET",
      url: 'https://forkify--api.herokuapp.com/getLikedRecipes',
      headers: {
        'Authorization': window.localStorage.getItem('forkifyToken')
      }
    }).done(result => {
  ...
  deleteLikedRecipe(id) {
    ...
    return $.ajax({
      type: 'DELETE',
      url: `https://forkify--api.herokuapp.com/deleteLikedRecipe/${recipeId}`,
      headers: {
        'Authorization': window.localStorage.getItem('forkifyToken')
      }
    }).done(result => {
